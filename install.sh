...
cat > "${INSTALL_DIR}/pam_face.py" << 'PAMEOF'
"""PAM integration module for ubuntu-face-login.

Invoked by pam_python.so. Calls the wrapper script to authenticate.

Accepts a timeout=N argument from the PAM config line, e.g.:
  auth sufficient pam_python.so /opt/ubuntu-face-login/pam_face.py timeout=2
...
_WRAPPER = "/usr/local/bin/ubuntu-face-login"
_DEFAULT_TIMEOUT = 5


def pam_sm_authenticate(pamh, flags, argv):
    syslog.openlog("pam_face_login")

    # Parse timeout=N from PAM config arguments
    ...
    # Resolve the target user from PAM
    try:
        user = pamh.get_user(None)
    except Exception:
        user = None

    cmd = [_WRAPPER, "auth", "--timeout", str(timeout)]
    if user:
        cmd += ["--user", user]
    ...
    try:
        result = subprocess.run(
            ...
            timeout=timeout + 5,  # hard kill margin
        )

        stdout = result.stdout.decode().strip()
        stderr = result.stderr.decode().strip()
        if stdout:
            syslog.syslog(syslog.LOG_INFO, f"stdout: {stdout}")
        if stderr:
            syslog.syslog(syslog.LOG_WARNING, f"stderr: {stderr}")

        if result.returncode == 0:
            syslog.syslog(syslog.LOG_INFO, "Face authentication succeeded")
            return pamh.PAM_SUCCESS
        else:
            syslog.syslog(syslog.LOG_WARNING, "Face authentication failed")
            return pamh.PAM_AUTH_ERR

    except subprocess.TimeoutExpired:
        syslog.syslog(syslog.LOG_ERR, "Face authentication timed out")
        return pamh.PAM_AUTH_ERR
    except Exception as e:
        syslog.syslog(syslog.LOG_ERR, f"Exception in pam_face.py: {e}")
